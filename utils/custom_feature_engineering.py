...
class CustomFeatureEngineering:
    ...
    def _create_customer_features(self):
        ...
        for i, (userid,value) in enumerate(self.df.groupby('userID')):
            ...
            # Pct_TransIDByDineIn
            self.customer_features.iat[i,8] = value[value['Channel'] == 'Dine In']['transID'].nunique() / self.customer_features.iat[i,1]

            # Pct_TransIDByTakeAway
            self.customer_features.iat[i,9] = value[value['Channel'] == 'Take Away']['transID'].nunique() / self.customer_features.iat[i,1]
            

            # === CHANNEL ===
            # Count_DistinctSource
            self.customer_features.iat[i,10] = value['OrderFrom'].nunique()

            # Pct_TransIDByStore
            self.customer_features.iat[i,11] = value[value['OrderFrom'] == 'STORE']['transID'].nunique() / self.customer_features.iat[i,1]

            # Pct_TransIDByWebsite
            self.customer_features.iat[i,12] = value[value['OrderFrom'] == 'WEBSITE']['transID'].nunique() / self.customer_features.iat[i,1]

            # Pct_TransIDByCallCenter
            self.customer_features.iat[i,13] = value[value['OrderFrom'] == 'CALL CENTER']['transID'].nunique() / self.customer_features.iat[i,1]

            # Pct_TransIDByApp
            self.customer_features.iat[i,14] = value[value['OrderFrom'] == 'APP']['transID'].nunique() / self.customer_features.iat[i,1]
...
if __name__ == "__main__":
    ...
    try:
        ...
        features._export_feature_files(output_folder=f"{Path(__file__).resolve().parent.parent/'output'}",
                                       customer_feature_name='customer_features_custom',
                                       customer_feature_transform_name='customer_feature_transformed_custom')
